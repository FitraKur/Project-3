...
def insert_raw_data():
  ...
  df.to_sql(name='data_raw_covid', con=engine, if_exists="replace", index=False)
  engine.dispose()
...
def insert_raw_to_warehouse(schema):
    mysql_auth = MySQL(credential['mysql_db'])
    engine, engine_conn = mysql_auth.connect()
    data = pd.read_sql(sql='data_raw_covid', con=engine)
    engine.dispose()
...
if __name__ == '__main__':
  ...
  insert_raw_to_warehouse(schema='public')